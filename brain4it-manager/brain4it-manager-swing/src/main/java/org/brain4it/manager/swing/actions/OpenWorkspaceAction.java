...
public class OpenWorkspaceAction extends ManagerAction
{
  public OpenWorkspaceAction(ManagerApp managerApp)
  {
    ...
    putValue(AbstractAction.NAME, 
      managerApp.getLocalizedMessage("OpenWorkspace"));
  }

  @Override
  public void actionPerformed(ActionEvent e)
  {
    if (managerApp.closeTabs())
    {
      managerApp.askSaveWorkspace();
      JFileChooser fileChooser = new JFileChooser();
      fileChooser.setFileFilter(new WorkspaceFileFilter());
      fileChooser.setDialogTitle(managerApp.getLocalizedMessage("OpenWorkspace"));
      int result = fileChooser.showDialog(
        managerApp, managerApp.getLocalizedMessage("Open"));
      if (result == JFileChooser.APPROVE_OPTION)
      {
        managerApp.loadWorkspace(fileChooser.getSelectedFile());
      }
    }
  }
}
