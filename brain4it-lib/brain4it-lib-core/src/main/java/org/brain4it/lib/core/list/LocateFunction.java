 ...
 * @author realor
 */
public class LocateFunction implements Function
{
  @Override
  public Object invoke(Context context, BList args) throws Exception
  {
    ...
    int index = args.size() > 4 ? 
      (Utils.toNumber(context.evaluate(args.get(4)))).intValue() : 0;
    boolean found = false;
    while (index < list.size() && !found)
    {
      Object element = list.get(index);
      context.setLocal(reference, element);
      if (Utils.toBoolean(context.evaluate(condition))) found = true;
      else index++;
    }
