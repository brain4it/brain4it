...
public abstract class AndroidFunction implements Function
{
  ...
  protected final Module getModule(Context context)
  {
    return (Module)context.getGlobalScope();
  }

  protected final Map<String, Function> getFunctions()
  {
    AndroidService androidService = AndroidService.getInstance();
    HttpServer server = androidService.getHttpServer();
    RestService restService = server.getRestService();
    ModuleManager moduleManager = restService.getModuleManager();
    return moduleManager.getFunctions();
  }  
}
