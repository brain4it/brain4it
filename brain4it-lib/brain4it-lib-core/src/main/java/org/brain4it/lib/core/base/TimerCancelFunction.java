...
public class TimerCancelFunction implements Function
{
  ...
  public TimerCancelFunction(CoreLibrary library)
  {
  ...
  public Object invoke(Context context, BList args) throws Exception
  {
    ...
    if (args.size() > 1)
    {
      ...
      int taskId = Utils.toNumber(value).intValue();

      Task task = library.getTask(taskId);
      if (task != null && task.getGlobalScope() == context.getGlobalScope())
      {
        library.removeTask(taskId);
        task.cancel();
        cancelled = true;
      }
    }
    else
    {
      ...
      {
        library.removeTask(task.getTaskId());
        task.cancel();
        cancelled = true;
      }
    }
