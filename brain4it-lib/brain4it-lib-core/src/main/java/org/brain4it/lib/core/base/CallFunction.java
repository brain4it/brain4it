...
public class CallFunction implements Function
{
  ...
  public Object invoke(Context context, BList args) throws Exception
  {
    ...
    {
      BHardReference reference = (BHardReference)value;
      BList newArgs = new BList(args.size() - 1);
      for (int i = 1; i < args.size(); i++)
      {
        newArgs.add(args.get(i));
      }
      Function function = (Function)reference.getFunction();
      result = function.invoke(context, newArgs);
    }
    else if (value instanceof BList)
    {
      ...
      {
        result = context.invokeUserFunction(list, args, 2);
      }
      else
      {
        result = value;
