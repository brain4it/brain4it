...
public class DuplicateServerAction extends ManagerAction
{
  ...
  {
    super(managerApp);
    putValue(AbstractAction.NAME, managerApp.getLocalizedMessage("DuplicateServer"));

    ImageIcon icon = IconCache.getIcon("duplicate");
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    Server srcServer = (Server)serverNode.getUserObject();
    
    // Create new Server object from the selected one
    Server dupServer = new Server(workspace);
    dupServer.setName(srcServer.getName() + " " + managerApp.getLocalizedMessage("DuplicatedServerSuffix"));
    dupServer.setAccessKey(srcServer.getAccessKey());
    dupServer.setUrl(srcServer.getUrl());
    ...
    workspace.getServers().add(dupServer);
    DefaultMutableTreeNode dupServerNode = new DefaultMutableTreeNode(dupServer);
    workspaceNode.add(dupServerNode);
    model.nodesWereInserted(workspaceNode,
      new int[]{workspaceNode.getChildCount() - 1});
   TreePath path = new TreePath(dupServerNode.getPath());
