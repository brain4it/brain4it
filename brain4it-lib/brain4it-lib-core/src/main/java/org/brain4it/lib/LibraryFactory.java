...
public class LibraryFactory
{
  ...
  private static final String PKG =
    LibraryFactory.class.getPackage().getName() + ".";

  @SuppressWarnings("unchecked")
  public static Class<? extends Library> getLibraryClass(String libraryName)
    throws ClassNotFoundException
  {
    libraryName = libraryName.trim();
    if (!libraryName.endsWith(LIBRARY_SUFFIX))
    {
      libraryName += LIBRARY_SUFFIX;
    }
    if (!libraryName.contains("."))
    {
      libraryName = PKG + libraryName;
    }
    return (Class<? extends Library>)Class.forName(libraryName);
  }
